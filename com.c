...
void null_command(FILE *stream, int count) {
    fprintf(stream, "Error: Null or empty command in execution attempt #%d\n", count);
    /* Customize the error message or take other actions as needed*/
}
...
void _path(char **commands, char **env) {
    ...
    while (token != NULL) {
        /* Build the full path to the executable by concatenating the directory and the command*/
        char *full_path = malloc(strlen(token) + 1 + strlen(commands[0]) + 1);
        if (full_path == NULL) {
            perror("malloc");
            free(path_copy);
            return;

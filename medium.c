...
int fk_unset_alias(fk_info_t *fk_info, char *fk_str)
{
	...
	c = *p;
	*p = 0;
	ret = delete_node_at_index(&(fk_info->fk_alias), get
			_node_index(fk_info->fk_alias, node_starts
				_with(fk_info->fk_alias, fk_str, -1)));
	*p = c;
	return (ret);
}
...
int fk_myalias(fk_info_t *fk_info)
{
	...
	for (i = 1; fk_info->fk_argv[i]; i++)
	{
		...
		if (p)
			...
			fk_print_alias(node_starts_with(
						fk_info->fk_alias, fk_info->fk_argv[i], '='));
	}
